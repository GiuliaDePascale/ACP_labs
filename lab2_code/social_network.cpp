...
void SocialNetwork::AddUser(const std::string &name,
  ...
  /* YOUR CODE GOES HERE */
  int index = CUserIndex(name, surname);
  if (index == users.size()) {
...
void SocialNetwork::AddFriendship(const std::string &first_name,
  ...
  /* YOUR CODE GOES HERE */
  int index1 = CUserIndex(first_name, first_surname);
  int index2 = CUserIndex(second_name, second_surname);
